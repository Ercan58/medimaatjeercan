...
# connect to database
def db_connect():
    return sqlalchemy.engine.create_engine('postgresql+psycopg2://medimaatjeadmin@medimaatje-auth:9050a3a086206329784301b99f410b03!@medimaatje-auth.postgres.database.azure.com:5432/medimaatje-auth?sslmode=require')

class Patient(Base):
    __table__ = Table(
        'patients', Base.metadata,
        Column('patient_user_id', Integer, ForeignKey('auth_user.id'), primary_key=True),
        Column('coach_user_id', Integer, ForeignKey('coaches.id')),
...
class Coach(Base):
    __table__ = Table(
        'coaches', Base.metadata,
        Column('id', Integer, ForeignKey('auth_user.id'), primary_key=True),
        Column('workplace', String(50))
    )

class User(Base):
    __table__ = Table('auth_user', Base.metadata, autoload=True, autoload_with=db_connect())
...
class DiseaseProfile(Base):
    __table__ = Table(
        'disease_profiles', Base.metadata,
